...
export class UserPreferenceLearningService {
   ...
   */
  async seedPreferencesFromInterests(userId: number, interests: string[]): Promise<void> {
   ...
   */
  async getTopPreferences(userId: number, limit: number = 10) {
   ...
   */
  private async invalidateFeedCache(userId: number): Promise<void> {
    // Delete all feed cache keys for this user
    // Pattern: feed:personalized:{userId}:page:*
    const keys = await this.cacheManager.store.keys(`feed:personalized:${userId}:*`)
