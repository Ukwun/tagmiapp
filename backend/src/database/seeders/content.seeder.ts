...
export async function seedContent(dataSource: DataSource, userMap: Map<string, User>): Promise<Content[]> {
  ...
  for (const user of users) {
    ...
    for (let i = 0; i < 20; i++) {
      // Determine content type (60% images, 25% videos, 10% audio, 5% text)
      const rand = Math.random()
      let contentType: "image" | "video" | "audio" | "text"
      ...
      // Create content with varied created dates (last 30 days)
