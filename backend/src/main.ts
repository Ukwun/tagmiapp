...
async function bootstrap() {
  ...
  app.enableCors({
    origin: (origin, callback) => {
      ...
      // Log origin for easier debugging with physical Android devices
      if (!origin || allowedOrigins.includes(origin)) {
        if (!origin) console.log('📱 Request received from mobile/local device');
  ...
  // Bind to 0.0.0.0 to allow physical Android devices on the same network to connect
