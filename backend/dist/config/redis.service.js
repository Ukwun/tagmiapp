...
let RedisService = RedisService_1 = class RedisService {
    ...
    async drainSignals(batchSize = 500) {
        if (!this.isConnected)
            return [];
        const pipeline = this.client.pipeline();
    ...
    async setCachedFeed(key, data, ttlSeconds) {
        if (!this.isConnected)
            return;
    ...
    async setCachedScores(scores, ttlSeconds = 600) {
        if (!this.isConnected)
            return;
        const pipeline = this.client.pipeline();
